...
def build_query(keywords):
    like_clauses = " OR ".join(["title LIKE ? OR content LIKE ?" for _ in keywords])
    params = []
    for word in keywords:
        wildcard = f"%{word}%"
        params.extend([wildcard, wildcard])
    return f"SELECT title, category, content FROM survival_guide WHERE {like_clauses}", params

def search_survival_guide(term, output_widget):
    keywords = term.lower().split()
    if not keywords:
        return
    query, params = build_query(keywords)
...
def load_titles():
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    cursor.execute("SELECT id, title FROM survival_guide WHERE title IS NOT NULL AND title != '' ORDER BY title ASC")
    results = cursor.fetchall()
    conn.close()
    return results
...
root = tk.Tk()
root.title("Offline Survival AI with Bible + Smart Search")
